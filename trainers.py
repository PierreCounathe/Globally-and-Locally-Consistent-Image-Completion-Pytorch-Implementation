...
def train_C(model_c, optimizer = opt_c, train_set = train_set, learning_rate = 1.0, train_acc_period = 100,
            n_epoch = 5, save_period = 10000, batch_size = 8, num_samples = 1000, use_cuda = True, pixel = (0, 0, 0), dataset_with_labels = True) :
  """
...
def train_C_and_D(model_c, model_d, alpha, optimizer_c = opt_c, optimizer_d = opt_d, train_set = train_set, lr_c = 1.0, lr_d = 1.0 ,train_acc_period = 100,
  ...
  * inputs :
  ...
  for epoch in tqdm(range(n_epoch)) :
    ...
    for data in tqdm(train_loader) : # There is no need for labels in the training dataset, but we let users choose if they want to use labelized datasets
      if dataset_with_labels:
        x, _ = data
      else:
        x = data
      ...
      loss_d = alpha * (loss_real + loss_fake) # Using a different coefficient for each kind of losses could be a way to personalize the net
      loss_d.backward()
      opt_d.step()
      ...
      input_ld_fake = hole_cropping(input_gd_fake,centers_c,use_cuda = use_cuda)
      output_fake = model_d((input_ld_fake,input_gd_fake))
      loss_c_2 = criterion(output_fake,real) # Indeed, the completion network must lure the discrimination network, hence this loss
